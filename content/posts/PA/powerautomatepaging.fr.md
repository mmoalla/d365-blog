...
![](images/PA/image1.png)

## Construire le squelette de la boucle
 
Ajoute une action **Do until**, avec comme condition : `MoreRecords` est égal à `false`.
![](images/PA/image2.png)
...
## Ajoute une condition pour vérifier s'il y a des données.
...
4. Incrémente la variable **`PageNumber`** de 1.
5. Mettre à jour la variable **`MoreRecords`** à *true*.
6. Incrémente la variable **`RecordsCount`** de la longueur du tableau de résultats (`length(outputs('List_rows')?['body/value'])`).

==Si la condition est à FALSE==
Mettre à jour la variable **`MoreRecords`** à *false* ce qui permet d'arrêter la boucle `Do until`.

![](images/PA/image4.png)

## Traiter les résultats de chaque page
